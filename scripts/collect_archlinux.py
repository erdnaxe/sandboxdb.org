...
from utils.archlinux_source import (
    get_pkg_url_containing_services,
    get_services_from_pkg,
)
from utils.service_json import update_service_json

mirror = "https://mirrors.gandi.net/archlinux"
arch = "x86_64"
for repository in ["core", "extra", "community"]:
    for name, url in get_pkg_url_containing_services(mirror, repository, arch):
        for service_name, service_content in get_services_from_pkg(url):
            data_path = f"./data/service/{service_name}.json"
            source_name = f"https://archlinux.org/packages/{repository}/{arch}/{name}/"
